...
export default function PracticePage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await apiGet<TodayPracticeResponse>("/api/practice/today");
  ...
  useEffect(() => {
    ...
    if (!item) return;
    apiGet<VocabularyDetailResponse>(`/api/vocabulary/${item.vocabularyId}`)
      .then((d) => setDefinition(d.item.definition))
      .catch(() => setDefinition("Definition not found"));
  ...
  async function handleReview(remembered: boolean) {
    ...
    setReviewing(true);
    try {
      await apiPost(`/api/practice/${session.id}/items/${item.id}/review`, {
        remembered,
      });
      if (currentIndex + 1 >= session.items.length) {
        await apiPost(`/api/practice/${session.id}/complete`, {});
        setCompleted(true);
      } else {
  ...
  const item: PracticeItem = session.items[currentIndex];
